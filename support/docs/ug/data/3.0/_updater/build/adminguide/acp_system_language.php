<?php
		// The page title
		$page_title = '3.10.4. Language Packs';

$navigation = array(
	'home' => array('../index.php','phpBB 3.0 Olympus Documentation'),
	'this' => array('../adminguide/acp_system_language.php','Language Packs'),
	'prev' => array('../',''),
	'next' => array('../',''),
...
ob_start();
?>
<div xmlns="http://www.w3.org/1999/xhtml" class="section" lang="en"><div class="titlepage"><div><div><h3 class="title"><span xmlns="" id="acp_system_language"/>3.10.4. Language Packs</h3></div><div><div class="authorgroup"><div class="author"><h3 class="author"><span class="othername">ameeck</span></h3></div></div></div></div></div><p>In phpBB, you can upload several language packs for your users to use. Every text string displayed by the system should be translated after you upload a new language pack. Using a different language pack does not change the contents of posts, as translating them is not possible due to the various content they can have and the limits of computer translation.</p><p>Language packs can be downloaded from the Downloads page on phpBB.com. To add a language pack, unzip the downloaded file and upload the contents to the <code class="filename">language/</code> directory. The files should be contained in a directory named with the language's ISO code, the default British English pack is in the <code class="filename">en/</code> directory for example. After you upload the files to the system, the pack should show in the <span class="guilabel">Uninstalled language packs</span> list. Click <span class="guilabel">Install</span> to add it to the board and make it available for users.</p><div class="note" style="margin-left: 0.5in; margin-right: 0.5in;"><h3 class="title">Note</h3><p>To make a language pack the default language of the board, you need to change the <a href="acp_general_config.php" title="3.2.1. Board Configuration">Board Configuration</a>.</p></div><p>If you are not comfortable with editing PHP files manually and you would like to change some text phpBB is displaying, you can use the in-built language pack editor which is accessible once you click the language pack title in the <span class="guilabel">Installed language packs</span> list. You can change the language pack details and the contents of individual language files on this page. In case you have more language packs installed and you select another language pack than the default one, you will see a list of untranslated variables if the packs are not synchronised. This is useful when you install MODs for one language and you cannot find what language strings you are missing in the other ones for example.</p><p>If you choose to edit a file using the ACP you have two options on how to update the translation on the board. First you can choose to <span class="guilabel">Submit</span> or <span class="guilabel">Submit and download</span> the file, which will store the file you are editing in the <code class="filename">store/</code> directory and the file will be offered for download if you choose the second button. If you choose to save the file in the store directory, you will have to manually move it to the language pack directory. The second possibility, which is right under these two buttons is to move the file to the language pack directory by using FTP. You will be prompted for FTP login credentials and if possible the file be saved and updated immediately by the script.</p></div><div class="copyright">© 2006, 2008 phpBB Group — Licensed under the Creative Commons <a href="http://creativecommons.org/licenses/by-nc-sa/3.0/">Attribution-NonCommercial-ShareAlike 3.0</a> license</div>
<?php
